...
- The default reviewer payload is `redacted`, which preserves placeholders and applies a second transient strict redaction pass to agent-visible response strings before reviewer calls. Use `restored` only when the reviewer endpoint is trusted to see private content.
...
- Structured access logs are disabled by default. When enabled, they log request metadata such as remote address, request id, and the standard HTTP log message; they do not log request or response bodies.
